...
typedef struct {
	...
	hal_bit_t *inch_icon;
	hal_bit_t *home_icon;
	hal_bit_t *height_setting_icon;
	hal_bit_t *run_icon;
	hal_bit_t *pause_icon;
	hal_bit_t *un40_icon;
	hal_bit_t *un04_icon;
	hal_bit_t *un08_icon;
} xhc_hal_t;
...
void xhc_display_encode(xhc_t *xhc, unsigned char *data, int len)
{
	...
	*p++ = 0x0C;
    ...
    if (*(xhc->hal->inch_icon)) {
        buf[FLAGS_BYTE] |= DISPLAY_INCH_ICON;
    }
    if (*(xhc->hal->home_icon)) {
        buf[FLAGS_BYTE] |= DISPLAY_HOME_ICON;
    }
    if (*(xhc->hal->height_setting_icon)) {
        buf[FLAGS_BYTE] |= DISPLAY_HEIGHT_SETTING_ICON;
    }
    if (*(xhc->hal->run_icon)) {
        buf[FLAGS_BYTE] |= DISPLAY_RUN_ICON;
    }
    if (*(xhc->hal->pause_icon)) {
        buf[FLAGS_BYTE] |= DISPLAY_PAUSE_ICON;
    }
    if (*(xhc->hal->un40_icon)) {
        buf[FLAGS_BYTE] |= DISPLAY_UNKNOWN_40;
    }
    if (*(xhc->hal->un04_icon)) {
        buf[FLAGS_BYTE] |= DISPLAY_UNKNOWN_04;
    }
    if (*(xhc->hal->un08_icon)) {
        buf[FLAGS_BYTE] |= DISPLAY_UNKNOWN_08;
    }
...
static void hal_setup()
{
    ...
    r |= _hal_pin_bit_newf(HAL_IN,  &(xhc.hal->home_icon), hal_comp_id, "%s.home-icon", modname);
    r |= _hal_pin_bit_newf(HAL_IN,  &(xhc.hal->height_setting_icon), hal_comp_id, "%s.height-setting-icon", modname);
    r |= _hal_pin_bit_newf(HAL_IN,  &(xhc.hal->run_icon), hal_comp_id, "%s.run-icon", modname);
    r |= _hal_pin_bit_newf(HAL_IN,  &(xhc.hal->pause_icon), hal_comp_id, "%s.pause-icon", modname);
    r |= _hal_pin_bit_newf(HAL_IN,  &(xhc.hal->un40_icon), hal_comp_id, "%s.40-icon", modname);
    r |= _hal_pin_bit_newf(HAL_IN,  &(xhc.hal->un04_icon), hal_comp_id, "%s.04-icon", modname);
    r |= _hal_pin_bit_newf(HAL_IN,  &(xhc.hal->un08_icon), hal_comp_id, "%s.08-icon", modname);
